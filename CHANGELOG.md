...
### 新增
- **启动图矢量重绘**：背景与文字分离——资源仅保留纯背景图，标题/副标题/分隔线/版本/版权全部运行时由 QPainter 矢量绘制（任意 DPI 清晰）；版本号与版权年份改为编译宏动态读取（不再烧录过期版本）；副标题可翻译
- **对话框布局守卫**：新增应用级 `DialogSizeGuard` 事件过滤器——对话框"设计初始尺寸 < 内容最小尺寸"时不再被布局压缩导致文字切半/控件重叠，自动放大到布局最小值；无顶层布局（子控件绝对定位）的对话框在子控件呈干净垂直堆叠时于显示时接管为垂直堆叠布局
- **全弹窗按钮样式统一**：对话框内约 100 个 QToolButton 批量改为 QPushButton（67 个 .ui 节点 + 约 33 处代码创建点，含色板按钮），消除 macOS 原生风格下两种按钮样式混排；对话框外的图标型工具按钮（齿轮弹出菜单、折叠头、查找/替换条）保留原样

### 修复
- **排序键下拉未翻译**：BOM/PLI 排序键（Part Color/Part Category/Part Size/Part Element/No Sort）显示为中文（零件颜色/零件类别/零件尺寸/零件元素/不排序）；显示文与 LPub 令牌分离（userData 携带令牌），文件兼容性不受影响；顺带修复上游 PliSortGui 以枚举值当 combo 索引导致初始选中错行的缺陷
- **"打开方式程序设置"对话框**：硬编码 `QString("Browse...")` 补翻译；列宽局促修复——名称列被挤到仅显示词尾（"ogram 1"），图标列/名称/路径/槽位 spin 补最小宽度，对话框最小宽 500→640
- **Program %1 默认名翻译**：默认程序名原为写入 QSettings 的硬编码英文数据；改为可翻译默认值并增加迁移——旧设置中条目名恰为旧英文默认时就地替换为译文默认（用户自改名不受影响）
- 启动图等硬编码 tr() 包裹补漏；新增译文 10 条（store 7,440 条，validate 护栏全绿）

### 下载
- macOS（Apple Silicon）：myLPub3D-v2.6.1-macOS.zip
