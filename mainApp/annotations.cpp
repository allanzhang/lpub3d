...
bool Annotations::overwriteFile(const QString &file)
{
 ...
 bool Annotations::exportUserElementsFile() {
    ...
    if(file.open(QIODevice::WriteOnly | QIODevice::Text))
    {
        ...
        outstream << "# The Part element entries generated by myLPub3D are only a sample provided for your guidance." << lpub_endl;
        outstream << "#" << lpub_endl;
        ...
        outstream << "# in the Annotations class. myLPub3D will look in the extras subfolder for all parameter files, if a " << lpub_endl;
        outstream << "# parameter file is found, it will be loaded. If not found, myLPub3D will revert to the hard coded data." << lpub_endl;
        outstream << "# If codes.txt is not found locally, myLPub3D will attempt to download it from" << lpub_endl;
...
bool Annotations::exportBLColorsFile() {
    ...
    if(file.open(QIODevice::WriteOnly | QIODevice::Text))
    {
        ...
        outstream << "# coded in the Annotations class. myLPub3D will look in the extras subfolder for all parameter files, if a " << lpub_endl;
        outstream << "# parameter file is found, it will be loaded. If not found, myLPub3D will revert to the hard coded data." << lpub_endl;
        outstream << "# If codes.txt is not found locally, myLPub3D will attempt to download it from" << lpub_endl;
        outstream << "# " << VER_LPUB3D_LIBRARY_BLCODES_URL << lpub_endl;
        outstream << "#" << lpub_endl;
...
bool Annotations::exportTitleAnnotationsFile() {
    ...
    if(file.open(QIODevice::WriteOnly | QIODevice::Text))
    {
        ...
        outstream << "# Annotations are extracted from the part title and further processed in myLPub3D" << lpub_endl;
